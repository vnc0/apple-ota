...
void extractFile(char * File, char * Name, uint32_t Size, short Perms, char * ExtractCriteria) {
    ...
    char * prefix = "/tmp/fs/";
    char * outName = (char * ) malloc((strlen(prefix) + strlen(Name) + 1) * sizeof(char));
...
int YAAright(unsigned char * mmapped, int Pos, uint64_t extractedSize) {
    ...
    00000000 59 41 41 31 36 00 54 59 50 31 44 50 41 54 50 00 | YAA16.TYP1DPATP. |
        ...
        00000010 00 55 49 44 31 00 47 49 44 31 00 4 d 4 f 44 32 ed | .UID1.GID1.MOD2. |
        00000020 01 46 4 c 47 31 00 4 d 54 4 d 54 | 56 6 f 5 e 61 00 00 | .FLG1.MTMTVo ^ a.. |
        00000030 00 00 6 d c4 2 f 11 59 41 41 31 35 00 54 59 50 31 | ..m. / .
    #endif

    // nasty because I originally passed mmapped  + pos. *Sigh*
    ...
    while (pos < yaaLen) {
        ...
        if (strcmp((const char * ) & attr, "AFT") == 0) {
            //printf("LEN: %d", len);
            len -= '0';
        ...
        else if (strcmp((const char * ) & attr, "DAT") == 0) {
            if (len == 'A') {
                memcpy( & dataLen, mmapped + pos + 4, sizeof(short));
                len = 2;
            }
            if (len == 'B') {
                memcpy( & dataLen, mmapped + pos + 4, sizeof(uint32_t));
                len = 4;
            }

        }
        else if (strcmp((const char * ) & attr, "MTM") == 0) {

            if (len == 'T') len = 12;
            if (len == 'S') len = 8;
        }
        else if (strcmp((const char * ) & attr, "LNK") == 0) {
            next = 3;
            int l = 0;
            len = 0;
        ...
        else if (strcmp((const char * ) & attr, "PAT") == 0) {
            ...
            if ((len >= '0') && (len <= '9')) {
                ...
                fprintf(stderr, "Wrong length for attr %s ('%c' = 0x%x) at pos 0x%x\n", & attr, len, len, pos);
            }
            if (len > 8) {
                fprintf(stderr, "wrong length at pos %d\n", pos);
                return -3;
            }
            ...
            if (strcmp((const char * ) & attr, "TYP") == 0) {
                if (p) fprintf(stdout, "TYP: %c\t", attrVal);
                typ = mmapped[pos + 4];
            } else if (strcmp((const char * ) & attr, "MOD") == 0) {
                if (p) fprintf(stdout, "MOD: 0%o ", attrVal);
                mod = attrVal;
            }
            // else if (strcmp(&attr, "UID") == 0) { }
            //	else if (strcmp(&attr, "GID") == 0) { }
            //	else if (strcmp(&attr, "FLG") == 0) { }
            else if (p) fprintf(stdout, "%s:  %-3d ", (char * ) & attr, attrVal);
        }
    ...
    if (pos > yaaLen) {
        fprintf(stderr, "POS 0x%x > YAALEN 0x%x\n", pos, yaaLen);
        exit(1);
    }
...
void processFile(char * FileName) {
    ...
    while (pos + 3 * sizeof(struct entry) < fileSize) {
        ...
        if ((ent -> usually_0x210_or_0x110 != 0x210 && ent -> usually_0x210_or_0x110 != 0x110 && ent -> usually_0x210_or_0x110 != 0x310) || ent -> usually_0x00_00) {
            fprintf(stderr, "Corrupt entry (0x%x at pos %llu@0x%llx).. skipping\n", ent -> usually_0x210_or_0x110, pos, (uint64_t)(mmapped + pos));
            int skipping = 1;
